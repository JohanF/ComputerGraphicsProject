...
{
	float4x4 modelViewMatrix = viewMatrix * make_identity<float4x4>();
	float4x4 modelViewProjectionMatrix = projectionMatrix * modelViewMatrix;

	float4x4 carModelViewMatrix = viewMatrix * make_translation(make_vector(0.0f, 0.0f, 0.0f));
	float4x4 carModelViewProjectionMatrix = projectionMatrix * modelViewMatrix;
	...
	setUniformSlow(shaderProgram, "modelViewProjectionMatrix", carModelViewProjectionMatrix);
...
{
	glBindFramebuffer(GL_FRAMEBUFFER, shadowMapFBO), glViewport(0, 0, shadowMapResolution, shadowMapResolution);
...
void drawScene(void)
{
	...
	float4x4 lightProjMatrix = perspectiveMatrix(75.0f, 1.0, 5.0f, 10000.0f);
	//Draw shadow map
	...
	float4 lightPositionv4 = make_vector4(lightPosition, 1.0f);
	float4 viewSpaceLightPosition = viewMatrix * lightPositionv4;
	...
	float4x4 lightMatrix = make_translation(make_vector(0.5f, 0.5f, 0.5f))*make_scale<float4x4>(0.5f)*((lightProjMatrix * lightViewMatrix * inverse(viewMatrix)));
	//make_scale<float4x4>() and make_translation().
	//We adjust the shadow map coordinates by scaling with 0:5 and translating with 0:5
