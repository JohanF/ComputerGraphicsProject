...
void main() 
{
	...
	vec3 envFinal = envMapSample * object_reflectiveness;
	...
	*/
	...
	vec3 ambient = material_diffuse_color * diffuse;
	vec3 directionToLight = normalize(viewSpaceLightPosition - viewSpacePosition);
	vec3 shading = calculateAmbient(scene_ambient_light, ambient) + calculateDiffuse(scene_light, diffuse, normal, directionToLight) * visibility + calculateSpecular(scene_light, specular, material_shininess, normal, directionToLight, directionFromEye) * visibility + emissive + calculateFresnel(specular, normal, directionFromEye) * envFinal * visibility;
	
	fragmentColor = vec4(shading, object_alpha);
	//fragmentColor = vec4(visibility * calculateDiffuse(scene_light, diffuse, normal, directionToLight), 1.0);
	//fragmentColor = vec4((diffuse + emissive)*visibility, object_alpha);
}
